 ...
 * Modified by YoungHoonKim on on 2016-11-19.
 */


/**
 클래스명 : public class MyDealingAdapter extends RecyclerView.Adapter<MyDealingAdapter.DerpHolder>
 주된기능 : MyDealingData의 정보를 어댑터를 통해 받아들인다.
 멤버변수 :
 private List<MyDealingListItem> listData : 내가 판 책들을 하나의 가변배열에 저장한것
...
public class MyDealingAdapter extends RecyclerView.Adapter<MyDealingAdapter.DerpHolder>{

    private List<MyDealingListItem> listData;
    private LayoutInflater inflater;

    private MyDealingAdapter.ItemClickCallback itemClickCallback;
    ...
    public MyDealingAdapter.DerpHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View view=inflater.inflate(R.layout.mydealing_list_item,parent,false);

        return new MyDealingAdapter.DerpHolder(view);
    }

    @Override
    public void onBindViewHolder(MyDealingAdapter.DerpHolder holder, int position) {
        MyDealingListItem item=listData.get(position);
        ...
        //holder.button.setImageBitmap(button);
    }
    ...
    class DerpHolder extends RecyclerView.ViewHolder implements View.OnClickListener{
        ...
        public DerpHolder(View itemView) {
            ...
            title=(TextView)itemView.findViewById(R.id.lbl_Ditem_book);
            writer=(TextView)itemView.findViewById(R.id.lbl_Ditem_writer);
            company=(TextView)itemView.findViewById(R.id.lbl_Ditem_company);
            type=(TextView) itemView.findViewById(R.id.lbl_Ditem_type);
            thumnail=(ImageView)itemView.findViewById(R.id.im_Ditem_icon);
            // icon2=(ImageView)itemView.findViewById(R.id.im_item_icon2);
            //icon2.setOnClickListener(this);
            container=itemView.findViewById(R.id.cont_Ditem_root);
        ...
        public void onClick(View v) {
            ...
            if(v.getId()==R.id.cont_Ditem_root) {
